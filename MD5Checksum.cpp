...
static void init_table(unsigned int* CRC32)
{
	int   i,j;
	unsigned int   crc;
	for(i = 0;i < 256;i++)
...
unsigned int crc32( BYTE *buf, int len)
{
...
CString GetCRC32(BYTE *buf, int len)
{
	CString TempStr;
	unsigned int crc=crc32(buf,len);

	TempStr.Format(_T("%08X"),crc);
...
CString GetLicense(CString DiskSN)
{
	CString	Md5Str=CMD5Checksum::GetMD5((BYTE*)DiskSN.GetBuffer(0),DiskSN.GetLength()*2);
	CString CrcStr=GetCRC32((BYTE*)DiskSN.GetBuffer(0),DiskSN.GetLength()*2);
	CString TempStr;

	TempStr += Md5Str.GetAt(10);
	TempStr += Md5Str.GetAt(30);
	TempStr += Md5Str.GetAt(20);
	TempStr += Md5Str.GetAt(26);
	TempStr += Md5Str.GetAt(9);
	TempStr += Md5Str.GetAt(1);
	TempStr += Md5Str.GetAt(18);
	TempStr += Md5Str.GetAt(24);
	TempStr += Md5Str.GetAt(5);
	TempStr += Md5Str.GetAt(22);
	TempStr += Md5Str.GetAt(7);
	TempStr += Md5Str.GetAt(15);

	TempStr += CrcStr.GetAt(5);
	TempStr += CrcStr.GetAt(2);
	TempStr += CrcStr.GetAt(7);
	TempStr += CrcStr.GetAt(4);
